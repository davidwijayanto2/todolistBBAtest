...
class EditTaskWidget extends StatefulWidget {
  final context;
  final docId;
  EditTaskWidget({this.context, this.docId});
  _EditTaskState createState() =>
      _EditTaskState(context: this.context, docId: this.docId);
}

class _EditTaskState extends StateMVC<EditTaskWidget> {
  _EditTaskState({this.context, this.docId}) : super(TaskController()) {
    cTask = controller;
  }
  final docId;
  ...
  void initState() {
    ...
    //setdatetimetext();
    cTask.getSingleTask(docId).listen((event) {
      setState(() {
        txttaskname.text = event.data()['taskname'];
        category = event.data()['category'];
        if (event.data()['category'] == 'Personal') {
          tvisibility[0] = false;
        } else if (event.data()['category'] == 'Work') {
          tvisibility[1] = false;
        } else if (event.data()['category'] == 'Meeting') {
          tvisibility[2] = false;
        } else if (event.data()['category'] == 'Study') {
          tvisibility[3] = false;
        } else if (event.data()['category'] == 'shopping') {
          tvisibility[4] = false;
        } else if (event.data()['category'] == 'Party') {
          tvisibility[5] = false;
        }
        var reminderdate =
            DateTime.parse(event.data()['reminder_at'].toDate().toString());
        date = DateFormat('yyyy-MM-dd').format(reminderdate);
        time = DateFormat('HH:00:00').format(reminderdate);
        datetime = DateTime.parse(date + ' ' + time);
